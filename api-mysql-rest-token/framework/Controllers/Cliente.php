        ...
        try {
            ...
            if ($method == "POST") {
                ...
                if (empty($_POST['nombres'])) {
                    $response = array('status' => false, 'msg' => 'Los nombres son requeridos');
                    jsonResponse($response, 200);
                    //die();
                }

                if (empty($_POST['apellidos'])) {
                    $response = array('status' => false, 'msg' => 'Los apellidos son requeridos');
                    jsonResponse($response, 200);
                    //die();
                }

                if (empty($_POST['telefono']) or !is_numeric($_POST['telefono'])) {
    ...
    public function actualizar($idcliente)
    {
        try {
            ...
            if ($method == "PUT") {
                ...
                if (empty($arrData['apellidos'])) {
                    $response = array('status' => false, 'msg' => 'Los apellidos son requeridos');
                    jsonResponse($response, 200);
                    die();
                }

                if (empty($arrData['telefono']) or !is_numeric($arrData['telefono'])) {
                    $response = array('status' => false, 'msg' => 'Error en el teléfono');
                    jsonResponse($response, 200);
                    die();
                }

                if (empty($arrData['email']) or !filter_var($arrData['email'], FILTER_VALIDATE_EMAIL)) {
                    $response = array('status' => false, 'msg' => 'Error en el email');
                    jsonResponse($response, 200);
                    die();
                }
